...
class fileSet:
	def __init__(self,listOfPartTypes):
		...
		self.OKPartNames = ['year2','year4','month','day','hour24start0','hour24start1','minute','second','millisecond']
		self.listOfPartTypes = copy.deepcopy(listOfPartTypes)


	
	def generateFilename(self, yearVal, monthVal, dayVal, hourVal, minuteVal, secondVal, millisecVal):
		'''Using your naming convention specified by the listOfPartTypes, create a name for a file starting at this time'''
		# a mapping of going from the part of the name to the string that should go to that part
		getNamePart = { 'year2': str(year2(yearVal)), 'year4': str(year4(yearVal)), 'month': str(month(monthVal)), 'day': str(day(dayVal,monthVal,yearVal)), 'hour24start0': str(hour24start0(hourVal)), 'hour24start1': str(hour24start1(hourVal)), 'minute': str(minute(minuteVal)), 'second':str(second(secondVal)) , 'millisecond':str(millisecond(millisecVal)) }
	...
	def generateFilenameDTObj(self, dateTimeObj):
		...
		return self.generateFilename(dateTimeObj.year, dateTimeObj.month, dateTimeObj.day, dateTimeObj.hour, dateTimeObj.minute, dateTimeObj.second, dateTimeObj.microsecond/1000)
	...
	def getTimeFromFilename(self, myFilename):
		...
		getValueVariable = {'year2': 'year' , 'year4': 'year', 'month': 'month', 'day': 'day' , 'hour24start0': 'hour' , 'hour24start1': 'hour' , 'minute': 'minute', 'second': 'second', 'millisecond':'millisecond'}
